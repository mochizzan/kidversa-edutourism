...
const create = async (data: CreateSessionDTO): Promise<Session> => {
  ...
  if (programStages.length === 0) {
    console.warn(`[sessions] Tidak ada program_stages untuk program ${data.program_id}; session akan dibuat tanpa stage`)
  }
...
const start = async (id: string): Promise<Session> => {
  const result = await update(id, { status: SessionStatus.ACTIVE })
  await liveService.initGroupProgress(id)
  return result
}
...
const createGroup = async (sessionId: string, name: string): Promise<SessionGroup> => {
  ...
  if (session?.status === SessionStatus.ACTIVE) {
    await liveService.initGroupProgress(sessionId)
  }
  return group
}
...
export const idbSessionService: SessionService = {
  getAll: getAll_,
  getById: getById_,
  create,
  update,
