...
import { ApiError, getApiBaseUrl } from '../../../core/services/backend-client'
...
const LearnerKioskPage = () => {
 ...
 const applyKiosk = (kiosk: KioskResponse) => {
  ...
  if (!stageId) {
   ...
   if (!first) {
    setError(t('learner.kiosk.noSessionContent'))
    return
   }
  ...
  if (!resolved) {
   setError(t('learner.kiosk.topicNotFound'))
   setLoading(false)
   return
  }
 ...
 const loadData = async () => {
  if (!sessionId) {
   setError(t('learner.kiosk.missingParams'))
   setLoading(false)
   return
  }
  if (!token) {
   setError(t('learner.kiosk.invalidToken'))
   setLoading(false)
   return
  }
  try {
 ...
 if (loading) {
  return (
   <div className="h-screen w-screen bg-black flex flex-col items-center justify-center text-white">
    ...
    <p className="text-lg">{t('learner.kiosk.loading')}</p>
   </div>
  )
 }
 ...
 if (error || !stage) {
  return (
   <div className="h-screen w-screen bg-surface flex flex-col items-center justify-center text-on-surface p-8 text-center">
    ...
    <h1 className="text-xl font-bold mb-2">{t('learner.kiosk.unavailableTitle')}</h1>
    <p className="text-on-surface-variant">{error || t('learner.kiosk.notFoundFallback')}</p>
   </div>
  )
 }
 ...
 if (locked) {
  return (
   <div className="h-screen w-screen bg-surface flex flex-col items-center justify-center text-on-surface p-8 text-center">
    ...
    <h1 className="text-xl font-bold mb-2">{t('learner.kiosk.lockedTitle')}</h1>
    <p className="text-on-surface-variant">{t('learner.kiosk.lockedDesc')}</p>
   </div>
  )
 }
 ...
 if (contents.length === 0) {
  return (
   <div className="h-screen w-screen bg-surface flex flex-col items-center justify-center text-on-surface p-8 text-center">
    <AlertTriangle className="w-16 h-16 text-warning mb-4" />
    <h1 className="text-xl font-bold mb-2">{t('learner.kiosk.emptyTitle')}</h1>
    <p className="text-on-surface-variant">{t('learner.kiosk.emptyDesc')}</p>
   </div>
  )
 }
 ...
 return (
  <div className="h-screen w-screen bg-black flex flex-col text-white">
   ...
   <div className="flex items-center justify-between px-6 py-4 bg-black/80">
    <div>
     <p className="text-xs text-white/60">{t('learner.kiosk.contentOf', { current: currentContentIndex + 1, total: contents.length })}</p>
     <h1 className="text-lg font-semibold">{currentContent?.title}</h1>
     {substage && <p className="text-xs text-white/50">{substage.status === 'COMPLETED' ? t('learner.kiosk.stageCompleted') : t('learner.kiosk.stageOngoing')}</p>}
    </div>
   ...
   <div className="flex items-center justify-center gap-2 pb-4">
    {contents.map((_, index) => (
     <button
      key={index}
      onClick={() => goTo(index)}
      className={`w-2 h-2 rounded-full transition-all ${index === currentContentIndex
       ? 'bg-primary w-6'
